...
def test_resnetarcface():
    ...
    if torch.cuda.is_available():
        ...
        net = ResNetArcFace(block='IRBlock', layers=(2, 2, 2, 2), use_se=True).to(mps_device).eval()
        img = torch.rand((1, 1, 128, 128), dtype=torch.float32).to(mps_device)
        output = net(img)
        assert output.shape == (1, 512)

        # -------------------- without SE block ----------------------- #
        net = ResNetArcFace(block='IRBlock', layers=(2, 2, 2, 2), use_se=False).to(mps_device).eval()
        output = net(img)
        assert output.shape == (1, 512)
...
def test_basicblock():
    if torch.cuda.is_available():
        ...
        """Test the BasicBlock in arcface_arch"""
        block = BasicBlock(1, 3, stride=1, downsample=None).to(mps_device)
        img = torch.rand((1, 1, 12, 12), dtype=torch.float32, device=mps_device)
        output = block(img)
        assert output.shape == (1, 3, 12, 12)

        # ----------------- use the downsmaple module--------------- #
        downsample = torch.nn.UpsamplingNearest2d(scale_factor=0.5).to(mps_device)
        block = BasicBlock(1, 3, stride=2, downsample=downsample).to(mps_device)
        img = torch.rand((1, 1, 12, 12), dtype=torch.float32, device=mps_device)
        output = block(img)
        assert output.shape == (1, 3, 6, 6)
...
def test_bottleneck():
    if torch.cuda.is_available():
        ...
        """Test the Bottleneck in arcface_arch"""
        block = Bottleneck(1, 1, stride=1, downsample=None).to(mps_device)
        img = torch.rand((1, 1, 12, 12), dtype=torch.float32, device=mps_device)
        output = block(img)
        assert output.shape == (1, 4, 12, 12)
        ...
        # ----------------- use the downsmaple module--------------- #
        downsample = torch.nn.UpsamplingNearest2d(scale_factor=0.5).to(mps_device)
        block = Bottleneck(1, 1, stride=2, downsample=downsample).to(mps_device)
        img = torch.rand((1, 1, 12, 12), dtype=torch.float32, device=mps_device)
        output = block(img)
        assert output.shape == (1, 4, 6, 6)
